...
class ArtN(Node):
	...
	def __eq__(self, other):
		return self.artNo==other.artNo
	def __hash__(self):
		return hash(self.artNo)
	def display(self):
		i = 1
...
class QueryNot(QueryStrategy):
	def query(self, tree, queryStr):
		def copyTree(node):
			newNode = None
			if type(node).__name__ == 'SetN':
				newNode = SetN(node.setVal, node.setCat)
				for ch in node.getChildren():
					c = copyTree(ch)
					newNode.addSubset(c)
					c.setParent(newNode)
			else:
				newNode = ArtN(node.artNo)
			return newNode
		queryStr = queryStr.lower()
		queryStr = queryStr.strip()[1:]
		cat = tree.checkLookup(queryStr)
		result = SetN('reroot', 'reroot')
		iter = Iterator(tree, queryStr, cat)
		child = copyTree(iter.newRoot)
		result.addSubset(child)
		child.setParent(result)
		notThese = set(result.getArticles())
		these = set(tree.root.getArticles()) - notThese
		result = tree.pruneTree(tree.root,these)
		return Tree(result, tree.lookup)
	...
	def query(self, tree, queryStr):
		...
		for quer in subquery:
			if '&' in quer:
				...
				subResult.addSubset(iter.newRoot)
				subResult = copyTree(subResult)
				result.addSubset(subResult)
				subResult.setParent(result)
		return Tree(result, tree.lookup)
...
class Tree():
				...
				if line[3]!='':
					...
					if line[4]!='':
						...
						if line[5]!='' or line[6]!='':
							...
							for name in names:
								...
								if orgs:
									for org in orgs:
										...
										tree[line[3]][line[4]][name][org].append(ArtN(i))
								
								else:
									if not emptyLabel in tree[line[3]][line[4]][name].keys():
										tree[line[3]][line[4]][name][emptyLabel] = []
									tree[line[3]][line[4]][name][emptyLabel].append(ArtN(i))
							...
							for org in orgs:
								...
								tree[line[3]][line[4]][org].append(ArtN(i))
						else:
							if not emptyLabel in tree[line[3]][line[4]].keys():
								tree[line[3]][line[4]][emptyLabel] = []
							tree[line[3]][line[4]][emptyLabel].append(ArtN(i))
						...
						for name in names:
							...
							if orgs:
								for org in orgs:
									...
									tree[line[3]][name][org].append(ArtN(i))
							
							else:
								if not emptyLabel in tree[line[3]][name].keys():
									tree[line[3]][name][emptyLabel] = []
								tree[line[3]][name][emptyLabel].append(ArtN(i))
					elif line[7]!='' and line[7]!='\n':
						line[7] = line[7].lower()
						orgs = line[7].split(';')
						for loop in range(len(orgs)):
						...
						for org in orgs:
							...
							if not org in tree[line[3]].keys():
								tree[line[3]][org] = []
							tree[line[3]][org].append(ArtN(i))
					else:
						if not emptyLabel in tree[line[3]].keys():
							tree[line[3]][emptyLabel] = []
						tree[line[3]][emptyLabel].append(ArtN(i))
				elif line[4]!='':
					line[4] = line[4].lower().strip()+";location"
					...
					if line[5]!='' or line[6]!='':
						...
						for name in names:
							...
							if orgs:
								for org in orgs:
									...
									tree[line[4]][name][org].append(ArtN(i))
							
							else:
								if not emptyLabel in tree[line[4]][name].keys():
									tree[line[4]][name][emptyLabel] = []
								tree[line[4]][name][emptyLabel].append(ArtN(i))
					elif line[7]!='' and line[7]!='\n':
						line[7] = line[7].lower()
						orgs = line[7].split(';')
						for loop in range(len(orgs)):
						...
						for org in orgs:
							...
							if not org in tree[line[4]].keys():
								tree[line[4]][org] = []
							tree[line[4]][org].append(ArtN(i))
					else:
						if not emptyLabel in tree[line[4]].keys():
							tree[line[4]][emptyLabel] = []
						tree[line[4]][emptyLabel].append(ArtN(i))
						...
						if orgs:
							for org in orgs:
								...
								if not org in tree[name].keys():
									tree[name][org] = []
								tree[name][org].append(ArtN(i))
						
						else:
							if not emptyLabel in tree[name].keys():
								tree[name][emptyLabel] = []
							tree[name][emptyLabel].append(ArtN(i))
				elif line[7]!='' and line[7]!='\n':
					line[7] = line[7].lower()
					orgs = line[7].split(';')
					for loop in range(len(orgs)):
					...
					for org in orgs:
						...
						if not org in tree.keys():
							tree[org] = []
						tree[org].append(ArtN(i))

				else:
					if not emptyLabel in tree.keys():
						tree[emptyLabel] = []
					tree[emptyLabel].append(ArtN(i))
				i += 1
		f.close()
	...
	def dfsDict(self, root, aSet, valList, newTreeDict, emptyLabel):
		if type(root).__name__=='SetN':
			if root.setVal!='reroot' or root.setVal!='root':
	...
	def runQuery(self):
		...
		while cont:
			...
			if queryStr!='':
				...
				print('No of articles:',len(finalTree.root.getArticles()))
