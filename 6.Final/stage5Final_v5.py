...
class SetN(Node):
	...
	def addSubset(self, Subset):
		if issubclass(Subset.__class__,Node):
			self.subsets.append(Subset)
		else:
			print('Error:Unacceptable Subset!')		#error/exception?
...
def dfsDict(root, aSet, valList, newTreeDict, emptyLabel):
	if type(root).__name__=='SetN':
		if root.setVal!='reroot':
			k = root.setVal+';'+root.setCat
			valList.append(k)
		for x in root.getChildren():
			dfsDict(x, aSet, valList[:], newTreeDict, emptyLabel)
	else:
		if root in aSet:
			k = newTreeDict
			for x in valList:
...
def pruneTree(root, aSet):
	emptyLabel = "<EMPTY LABEL>"
	newTreeRoot = {}
	dfsDict(root, aSet, [], newTreeRoot, emptyLabel)
	reroot = SetN('reroot','reroot')
	build_tree(reroot, newTreeRoot, emptyLabel)
	return reroot
...
class QueryStrategy:
	...
	def getQueryFn(self, queryStr):
		...
		def queryAnd(self, queryStr):
			...
			for s in setList:
				attrSet &= s
			result = pruneTree(result, attrSet)
			return result
		...
		def queryAndOr(self, queryStr):
			...
			for quer in subquery:
				if '&' in quer:
					...
					for s in setList:
						attrSet &= s
					subResult = pruneTree(subResult, attrSet)
...
def trav_root_list(roots,lvl):
	...
	for root in roots:
		if (type(root).__name__=='SetN') != isSet:
			print('Type of current node at level',lvl)
			return False
	ans = True
	if isSet:
		...
		for root in roots:
			if root.setVal!=v or root.setCat!=c:
				print('Current set node attributes mismatch at level',lvl)
				return False
		l = len(roots[0].getChildren())
		for root in roots:
			if len(root.getChildren())!=l:
				print('Current no of children mismatch at level',lvl)
				return False
		for i in range(l):
			li = []
			for root in roots:
		...
		for root in roots:
			if root.artNo!=a:
				print('Current article node mismatch at level',lvl)
				return False
	return ans
...
'''
queryStr = 'football'
tree.query = tree.querSys.getQueryFn(queryStr)

finalTree = tree.query(tree, queryStr)
finalTree.display()
if len(finalTree.getArticles())==0:
	print("\nYour query did not return any results.")
print("Continue? (Y/N)")
...
def disp_spec(node, disp):
	li = []
	if type(node).__name__=='SetN':
		if not disp:
			if node.setVal=='nadal':
				disp = True
		for no in node.getChildren():
			li.extend(disp_spec(no, disp))
	else:
		if disp:
			li.append(node)
	return li
naLi = disp_spec(finalTree, False)
for a in naLi:
	a.display()
'''

'''	t = tree['football;sport']

	na = []
	for tr in t.values():
		if type(tr).__name__!='list':
			if 'nadal;person' in tr.keys():
				for li in tr['nadal;person']:
					na.append(li)
	print('AAAAAAAAAAAAAAAAAAAHHHHHHHHHHHHHHHH',set(na))'''
